...
public interface StationRepository extends JpaRepository<Station, Integer> {
    ...
    @Query(value = "SELECT * FROM station s WHERE s.isActive =1 AND EXISTS (SELECT 1 FROM station_route sr1 "
    ...
    @Query(value = "SELECT * from station where isActive =1 AND latitude BETWEEN ?1 - 0.063555 AND ?1 + 0.063555 and "
