...
export default function EachElementLookup({search, user}) {
  ...
  const fetchData = async () => {
    ...
    domain = 'http://' + domain;
    const data = await routeService.getAllRoute(domain);
    if (user != '') {
      const data2 = await UserRouteLikesService.getAllByUserId(user.id, domain);
      setBonusRoute(data2);
    }
    setFakeRoute(data);
    setPage(page + 1);
  ...
  const fetchSearchData = async () => {
    ...
    domain = 'http://' + domain;
    const data = await routeService.getSearchRoute(search, domain);
    setFakeRoute(data);
    setPage(page + 1);
    setLoading(false);
  };
  ...
  const addNew = async item => {
    ...
    domain = 'http://' + domain;
    const rs = UserRouteLikesService.add(addItem, domain);
    if (rs == null) {
